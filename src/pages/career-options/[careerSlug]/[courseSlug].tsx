...
import { ApiResponseHandler, GetAllCareers, GetCareerCoursesDetails } from '@/Services/Api.service';
...
const getData = async (params) => {
  ...
  return await GetCareerCoursesDetails({ ...params });
}
...
export async function getStaticPaths() {
  ...
  careers.map((career) => {
    career?.courses.map((course) => {
      paths.push({
        params: { courseSlug: course.slug, careerSlug: career.slug },
      })
    })
  })
...
export async function getStaticProps({ params }) {
  ...
  let response = await getData({ careerSlug: params?.careerSlug, courseSlug: params.courseSlug });
  if (response) {
